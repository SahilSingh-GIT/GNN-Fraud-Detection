...
export default function App() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setTxHistory((prev) => [
        {
          amt: form.TransactionAmt,
          risk: data?.message ? 90 : Math.round((data?.probability || 0) * 100),
          fraud: data?.prediction === 1,
          time: new Date().toLocaleTimeString(),
        },
        ...prev.slice(0, 4),
      ]);
    } catch {
      setResult({ __error: true });
    }
    setLoading(false);
  };

  const getXAIFeatures = () => {
    const t = Number(form.TransactionAmt),
      c1 = Number(form.card1),
      c2 = Number(form.card2),
      c3 = Number(form.card3),
      a1 = Number(form.addr1),
      d1 = Number(form.dist1);
    return [
      {
        label: "Transaction Amount",
        value: Math.min(t / 1000, 1),
        importance: Math.min(t / 1000, 1),
      },
      {
        label: "Card Pattern 1",
        value: c1 > 9000 ? 0.95 : c1 < 2000 ? 0.7 : 0.2,
        importance: c1 > 9000 ? 0.95 : c1 < 2000 ? 0.7 : 0.2,
      },
      {
        label: "Card Pattern 2",
        value: Math.min(c2 / 800, 1) * 0.6,
        importance: Math.min(c2 / 800, 1) * 0.6,
      },
      {
        label: "Card Pattern 3",
        value: Math.min(c3 / 200, 1) * 0.5,
        importance: Math.min(c3 / 200, 1) * 0.5,
      },
      {
        label: "Address Score",
        value: Math.min(a1 / 1000, 1) * 0.8,
        importance: Math.min(a1 / 1000, 1) * 0.8,
      },
      {
        label: "Distance Anomaly",
        value: Math.min(d1 / 300, 1),
        importance: Math.min(d1 / 300, 1),
      },
  ...
  const getReasons = () => {
    ...
    if (t > 500)
      r.push({
        text: "Transaction amount exceeds safe threshold",
        severity: "high",
        icon: "💸",
      });
    if (a1 > 500)
      r.push({
        text: "Address risk score out of normal range",
        severity: "medium",
        icon: "📍",
      });
    if (d1 > 100)
      r.push({
        text: "Geographic distance anomaly detected",
        severity: "medium",
        icon: "🗺️",
      });
    if (c1 < 2000)
      r.push({
        text: "Irregular card behavioral pattern",
        severity: "high",
        icon: "💳",
      });
    if (c1 > 9000)
      r.push({
        text: "Unrecognized user — no historical baseline",
        severity: "critical",
        icon: "👤",
      });
    if (!r.length)
      r.push({
        text: "All parameters within safe thresholds",
        severity: "safe",
        icon: "✅",
      });
    return r;
  ...
  const S = {
    ...
    btn: (l) => ({
      ...
      letterSpacing: 4,
      textTransform: "uppercase",
      border: `1px solid ${l ? "rgba(255,215,0,0.15)" : "rgba(255,215,0,0.35)"}`,
      background: l
        ? "rgba(255,215,0,0.05)"
        : "linear-gradient(135deg,rgba(255,215,0,0.7),rgba(192,192,192,0.5))",
      color: l ? "#71717A" : "#000000",
      cursor: l ? "not-allowed" : "pointer",
      boxShadow: l
        ? "none"
        : "0 0 40px rgba(255,215,0,0.12),0 0 80px rgba(192,192,192,0.06),inset 0 1px 0 rgba(255,255,255,0.15)",
      transition: "all 0.4s",
      position: "relative",
      overflow: "hidden",
    }),
    ...
    footer: {
      ...
      color: "#52525B",
      letterSpacing: 3,
      borderTop: "1px solid rgba(255,215,0,0.04)",
    },
    emptyState: {
      display: "flex",
      flexDirection: "column",
    ...
    reasonRow: (s) => ({
      ...
      borderRadius: 12,
      background: sevBg(s),
      border: `1px solid ${sevColor(s)}20`,
      marginBottom: 8,
    }),
    ...
    <div style={S.page}>
      ...
      <div style={S.layer}>
          ...
          <div style={S.container}>
            ...
            <div style={S.titleWrap}>
              <div style={S.subtitle}>
                EXPLAINABLE AI · REAL-TIME GRAPH NEURAL NETWORK
              </div>
              <h1 style={S.h1}>
                Transaction{" "}
                <span style={S.h1Accent(riskColor)}>Intelligence</span>
              </h1>
              <div style={S.desc}>
                VISUALIZATION · SHAPLEY EXPLANATIONS · SUBGRAPH CONSTRUCTION
              </div>
            </div>
            ...
            <div style={S.grid}>
              ...
              >
                <GlassPanel
                  glowColor={hasResult ? riskColor : null}
                  animate={showResult}
                >
                  {!hasResult && !loading && (
                    <div style={S.emptyState}>
                      ...
                      <p style={S.emptyText}>
                        AWAITING TRANSACTION DATA FOR NEURAL ANALYSIS
                      </p>
                  ...
                  {loading && (
                    <div style={S.loadingWrap}>
                      ...
                      >
                        Constructing subgraph · Computing embeddings · Running
                        inference
                      </div>
                    </div>
                  )}
                      ...
                      <div>
                        ...
                        {reasons.map((r, i) => (
                          <div key={i} style={S.reasonRow(r.severity)}>
                            ...
                            <span style={S.reasonBadge(r.severity)}>
                              {r.severity}
                            </span>
                          </div>
                        ))}
                ...
                {showXAI && hasResult && (
                  <GlassPanel animate glowColor="#C0C0C0">
                    ...
                    <div style={S.xaiInfo}>
                      ℹ️ Shapley values represent each feature's marginal
                      contribution to the risk prediction. Higher bars indicate
                      stronger influence on the fraud classification decision.
                    </div>
                  </GlassPanel>
                )}
              </div>
        ...
        <footer style={S.footer}>
          NEURALGUARD · GRAPH NEURAL NETWORK FRAUD INTELLIGENCE · MODEL
